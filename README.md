これはElasticsearch 実践ガイドのまとめ
work ディレクトリ以下に、実際に作業する用の資料を残してある
...
# 3章(ドキュメント/インデックス/クエリの基本操作)
## 3-2 インデックスとドキュメントタイプの管理
* インデックスやドキュメントタイプを登録していなくても、POSTによってドキュメントを追加できる。
  * しかし、その場合変数がデフォルト値で決定されたり推論により決定されたりするので注意が必要
  * インデックスを明示的に事前に作成しておくべきケース
    * デフォルト値とは異なるレプリカ数やシャード数を持つインデックスを作成する場合
  * ドキュメントタイプを明示的に事前に作成しておくべきケース
    * 登録するドキュメント内のデータ型があらかじめわかっていて、かつ自動推測ではなく明示的にマッピングを指定したい場合
* インデックステンプレートを作成すると、特定の正規表現に一致するインデックスが作成されるときにそのインデックス設定をテンプレ通りに設定する、みたいなことができる

## 3-3 さまざまなクエリ
### 3-3-1
* クエリDSL に指定するもの
  * query: クエリの内容
  * from: 何個目から返すか？デフォルト0
  * size: 何個返すか
  * _source: 検索結果に含めたいフィールド_

### 3-3-3 クエリDSLの種類
* 全文検索クエリ
  * query 以下のレイヤーに↓を指定して検索をする
    * match_all: 全てのドキュメントを返す
    * match: 部分一致。複数をスペース区切りで指定すると OR 検索可能。
      * operator: and を指定すると AND 検索になる
      * minimum__should_match: これで指定した個数以上にヒットすれば返す
    * match_phrase: 語順も含めてチェックする
    * query_string: Lucene シンタックスでの検索式を直接かける
* Term ベースクエリ
  * term: 完全一致 "New York" とか。
  * terms: 完全一致の OR。["New York", "Hoge Hoge"] みたいな。
  * range: 値の範囲検索ができる。gteとかgtとかlteとかltとか指定する。日付だと 1y とか now とか指定可能。
* 複合クエリ、Bool クエリ
  * bool: 以下のレイヤーに↑の複合クエリじゃないやつをしていして、条件を組み合わせることができる。
    * must: AND にあたる
    * should: OR にあたる
    * must_not: NOT にあたる
    * filter: ↑3つはスコアに影響を与えるが、filter は足切りをしてくれる。
      * filter クエリはクエリキャッシュが使われるため、検索結果を各セグメント単位でメモリ上に保持する。性能向上が図れる


# 4章(Analyzer/Aggregation/スクリプティングによる高度なデータ分析)

## 4.1 全文検索と Analyzer
* Analyzer が文章を単語に分割する
* 代表的な日本語の Analyzer が kuromoji
* マッピング定義で指定可能
* Analize の流れ
  * Char filter(0〜n個): 前処理。HTMLフォーマットのタグ除去や、特定の文字の別の文字列への置換等。
  * Tokenizer(1個): 単語分割
  * Token filter(0〜n個): 分割されたトークン(単語)それぞれに対する処理。文字を全て小文字にしたり、ストップワードの除去をおこなったり、ステミング処理をしたり、シノニムを正規化したり。

## 4.2 Aggregation
* 直訳すると集約。
* 検索結果の集合に対して、分類や集計を行うことができる。
* 検索結果を Buckets で分割し、そのグループごとの統計値を Metrics で求める
  * RDS でいうと、group by で分割し、count でグループごとの個数を数える、みたいな感じ

## 4.3 スクリプティング
* 以下のような処理を行うことができる(が、前出の処理と比べて相対的に思い処理となる)。
  * フィールドの上書き更新
  * 特定のフィールドのみを一度に上書き更新する
  * インデックスに含まれるフィールドをすべて更新する
    * インデックスに含まれるフィールドを全て更新する操作を reindex と呼ばれるインデックス再作成操作とあわせておこなう
    * reindex とは、インデックスのマッピングや設定が変更された場合に、既存のインデックスに含まれるデータをコピーして、新規にインデックスを作り直す際に用いられる機能


# 5章 システム運用とクラスタの管理
## 5.4 インデックスの管理とメンテナンス
### 5.4.2 再インデックス
* シャード数を変えたいとき、別にインデックスを作成してそこに全てのドキュメントをコピーすることで実現できる。
* この全てのドキュメントのコピーを再インデックス(reindex)と呼ぶ

# 6章 Elastic Stack インテグレーション
## 6-1 Elastic Stack
### 6-1-1 Kibana
* Kibana ではユーザが作成したグラフやダッシュボードの構成情報を保存できるが、それらはすべて接続先の Elasticsearch 上のインデックスに格納される！！
* ので、Kibana 自体はステートレス。Kibana に障害が生じたら、別のサーバ上で Kibana を起動すれば良い。
* デフォルトで localhost の 5601 をリッスンする。localhost の 9200 で起動している Elasticsearch へ接続する動作をする
  * 変更するなら /etc/kibana/kibana.yml をいじれば良い。
